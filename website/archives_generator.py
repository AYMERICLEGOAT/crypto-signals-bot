...
# Les quatre familles diffusées depuis le 04/08/2026. Décrites ici plutôt qu'en
# dur dans le HTML pour que la page d'accueil et le balisage schema.org ne
# puissent pas diverger — c'est exactement ce qui avait laissé la page décrire
# un moteur qui n'était plus diffusé.
FAMILIES = [
    ("Force relative",
    ...
    ("Expansion de volatilité",
     ...
     "C'est la famille la plus rare des quatre.",
     "Directionnel"),
...
def _families_html():
    cards = "\n".join(
        ...
        <span class="tag{' gold' if tag == 'Neutre au marché' else ''}">{html.escape(tag)}</span>
      </article>"""
...
{_families_html()}
  ...
  <section>
    ...
    <p><a href="/comment-ca-marche.html">Comment ça marche</a> détaille les quatre familles étape par
