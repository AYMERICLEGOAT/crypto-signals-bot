...
def build_terms():
  ...
  <section>
    ...
    <p>Le service diffuse, de façon automatisée, des signaux d'analyse technique crypto sur
       40 paires USDT. Il comporte deux niveaux, et la différence entre les deux est le cœur de
       l'abonnement&nbsp;:</p>
    <ul>
      <li><strong>Abonnement payant</strong>&nbsp;: chaque signal est transmis avec ses niveaux —
          prix d'entrée, stop loss, objectifs et durée prévue — au moment où il est détecté.</li>
      <li><strong>Canal public gratuit</strong>&nbsp;: l'ouverture de chaque signal y est annoncée
          (paire, moteur, durée prévue) <em>sans ses niveaux</em>. Le signal complet — niveaux
          d'origine et résultat, gagnant ou perdant — y est publié à sa clôture, plus un signal
          complet offert par semaine. Le relevé public est donc entier et vérifiable&nbsp;; c'est
          le moment où il devient exploitable qui distingue l'abonnement.</li>
    </ul>
    <p>La stratégie classe
  ...
  <section>
    ...
    <p>L'état de cette condition (active ou non) est annoncé sur le canal et expliqué en détail sur
       ...
       incident technique (section 10) est une chose différente et sera signalée comme telle.</p>
  </section>

  <section>
    <h2>3. Le carry de financement — ce qu'il est, et ce qu'il n'est pas</h2>
    <p>Une partie des signaux diffusés sont des <strong>carrys de financement</strong>. Leur
       fonctionnement diffère de tout le reste, et les malentendus qu'ils peuvent produire portent
       sur de l'argent réel : cette section les traite séparément.</p>

    <p><b>Ce que le signal contient.</b> Un carry décrit <strong>deux positions à ouvrir
       simultanément et pour le même montant</strong> sur la même cryptomonnaie : un achat au
       comptant, et une vente à découvert du contrat perpétuel. Les deux s'annulent au regard du
       prix. Le résultat provient du financement versé toutes les 8 heures par les acheteurs de
       perpétuels aux vendeurs.</p>

    <div class="warn">
      <p><b>C'est à toi de les ouvrir, et sur ta propre plateforme.</b> Nous n'exécutons aucun
         ordre, ne détenons aucun fonds et n'avons accès à aucun de tes comptes. Un carry suppose
         d'avoir accès à un marché de contrats perpétuels, ce qui n'est pas le cas de toutes les
         plateformes ni de tous les pays.</p>
      <p><b>N'ouvrir qu'une seule jambe transforme une position neutre en pari directionnel.</b>
         C'est l'erreur la plus coûteuse possible sur ce type de signal, et elle est entièrement
         hors de notre contrôle. Les deux jambes doivent aussi être <strong>fermées ensemble</strong>.</p>
      <p><b>Le rendement annoncé n'est PAS acquis.</b> Le chiffre indiqué à l'ouverture est une
         estimation fondée sur le taux de financement observé à cet instant. Ce taux varie pendant
         la détention et peut s'inverser. Sur 6 ans de mesure, <strong>41 % des positions
         seulement</strong> ont atteint ou dépassé le montant annoncé à l'ouverture.</p>
      <p><b>« Neutre au marché » ne veut pas dire « sans risque », et nous ne l'écrirons jamais.</b>
         La jambe vendeuse peut être <strong>liquidée</strong> si ta marge devient insuffisante ; la
         plateforme sur laquelle tu opères peut faire défaut, geler les retraits ou modifier ses
         règles de financement ; et une période de financement extrême a coûté jusqu'à
         <strong>-19,86 %</strong> sur une position mesurée. Un carry n'a ni stop loss ni take
         profit : aucun niveau de prix ne le ferme, seule la date le fait.</p>
    </div>

    <p>Les chiffres de performance du carry publiés sur ce site (84,2 % de positions gagnantes,
       +0,572 % net par position, 6 années positives sur 7) proviennent d'une simulation sur données
       historiques, frais estimés déduits. Ils supposent une exécution parfaite des deux jambes, une
       marge toujours suffisante et l'absence de défaillance de plateforme — trois conditions que
       nous ne pouvons ni vérifier ni garantir dans ton cas. Ils relèvent intégralement de la
       section&nbsp;10 ci-dessous.</p>
  </section>

  <section>
    <h2>4. Accès et capacité à contracter</h2>
    <p>Le canal public gratuit et l'essai gratuit sont accessibles à toute personne disposant d'un
       compte Telegram. Les offres payantes (section 5) impliquent un paiement irréversible et
  ...
  <section>
    ...
    <table class="recent">
      ...
      <tbody>
        ...
        <tr><td>Mensuel</td><td>30 jours</td><td>19 USDT</td></tr>
        <tr><td>Trimestriel</td><td>90 jours</td><td>45 USDT, soit 15 USDT par mois</td></tr>
      </tbody>
    </table>
  ...
  <section>
    <h2>7. Absence de droit de rétractation</h2>
    <p>L'abonnement est un contenu numérique dont l'exécution démarre immédiatement dès
       ...
       confirmé — y compris en cas d'erreur d'envoi de ta part (section 6) et y compris si aucun
       signal n'est émis pendant la période souscrite (section 2).</p>
  </section>

  <section>
    <h2>8. Durée, non-reconduction et résiliation</h2>
  ...
  <section>
    <h2>11. Résiliation par l'éditeur</h2>
