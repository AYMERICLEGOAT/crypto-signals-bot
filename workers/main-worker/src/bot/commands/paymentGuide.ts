...
export async function handlePaymentGuideCommand(env: Env, telegramId: number): Promise<void> {
  const text = [
    ...
    "",
    "Ton accès s'active tout seul dans les 5 minutes qui suivent la confirmation du réseau.",
    "Cette confirmation prend moins d'une minute en USDT/Polygon, une dizaine de minutes en",
    "Litecoin, et une vingtaine en Monero — dix confirmations y sont exigées avant de créditer.",
