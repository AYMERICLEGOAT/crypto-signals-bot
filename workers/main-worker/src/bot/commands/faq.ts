...
export async function handleFaqCommand(env: Env, telegramId: number): Promise<void> {
  ...
  const service = [
    ...
    "Le nôtre est de 47,7 % : moins d'un signal sur deux est gagnant, et nous",
    "l'écrivons plutôt que de le maquiller. Ce qui compte est l'écart entre les",
    "deux côtés : gagnant moyen +16,88 %, perdant moyen -9,24 %, soit +3,22 %",
    "attendus par signal, net de frais.",
    "",
