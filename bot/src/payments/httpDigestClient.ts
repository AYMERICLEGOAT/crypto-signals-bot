 ...
 * `fetch()` ne gère pas nativement ce schéma — on l'implémente ici plutôt
 * que de recommander de désactiver l'authentification du wallet-rpc, ce
 * qui serait risqué dès lors que l'URL est exposée via ngrok.
 */
...
export async function digestAuthFetch(
  ...
  if (firstAttempt.status !== 401) {
    // --rpc-login désactivé côté wallet-rpc : pas d'authentification nécessaire.
    return firstAttempt;
  }
